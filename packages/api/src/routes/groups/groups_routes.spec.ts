...
vi.mock('@/core/repositories/groups/implementation/postgres_group_repository', () => ({
  PostgresGroupRepository: vi.fn(function () {
    return groupRepo
  }),
}))
...
async function build_app(): Promise<FastifyInstance> {
  ...
  await app.register(groups_routes)
  await app.ready()
...
describe('groups_routes', () => {
  ...
  describe('GET /grupos/:id', () => {
    it('returns grupo by id', async () => {
      const g = await groupRepo.create({ hotel_id: HOTEL_ID, grupo: 'G1' })
      const res = await app.inject({
        method: 'GET',
        url: `/grupos/${g.id}`,
        cookies: { token },
  ...
  describe('DELETE /grupos/:id', () => {
    ...
    it('returns 409 when grupo has linked users', async () => {
      const g = await groupRepo.create({ hotel_id: HOTEL_ID, grupo: 'G1' })
      groupRepo.setLinkedUsers(g.id, true)
      const res = await app.inject({
        method: 'DELETE',
        url: `/grupos/${g.id}`,
        cookies: { token },
